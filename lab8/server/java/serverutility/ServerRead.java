...
public class ServerRead extends RecursiveAction {
    ...
    public ServerRead(byte[] data, SocketAddress client,
                      ...
                      ServerCommand serverCommand, CollectionManager collectionManager, Set<SocketAddress> clients) {
        this.data = data;
        this.client = client;
    ...
    protected void compute() {
        try (ObjectInputStream ois = new ObjectInputStream(new ByteArrayInputStream(data))) {
            ...
            System.out.println(commandName + client);
            Object color = ois.readObject();
            if (color != null) {
                ...
                List<Object> response = serverCommand.execute(commandName, args, (String) color);
    ...
    private void sendResponse(List<Object> response) {
        // Отправляем результат команды клиенту
        senderPool.execute(new ServerAnswer(channel, client, response, collectionManager));

// Рассылаем остальным клиентам только коллекцию
        System.out.println("Clients" + clients);
        Set<SocketAddress> otherClients = clients.stream()
                .filter(c -> !c.equals(client))
                .collect(Collectors.toSet());
        System.out.println("OtherCl" + otherClients);
        Server.broadcastToAll(channel, otherClients, collectionManager, senderPool);
    }
}